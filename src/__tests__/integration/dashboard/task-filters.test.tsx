...
describe('Task Filters - Next Week and This Month', () => {
  ...
  describe('Next Week Filter', () => {
    it('displays "Next Week" filter button', () => {
    ...
    it('shows correct count for next week tasks', () => {
      const today = new Date();
    ...
    it('excludes skipped tasks from next week count', () => {
      const today = new Date();
  ...
  describe('This Month Filter', () => {
    it('displays "This Month" filter button', () => {
      vi.mocked(useApp).mockReturnValue({
      ...
      expect(monthFilters[0]).toBeInTheDocument();
    });

    it('shows correct count for this month tasks', () => {
      const today = new Date();
    ...
    it('filters tasks for this month when clicked', () => {
      const today = new Date();
      const endOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0);
    ...
    it('includes all remaining days of current month', () => {
    ...
    it('excludes skipped tasks from month count', () => {
      const today = new Date();
      const endOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0);
  ...
  describe('Filter Combination', () => {
    it('shows all filters in correct order', () => {
      vi.mocked(useApp).mockReturnValue({
        ...mockAppContext({ tasks: [] }),
        isGoogleCalendarConnected: () => false,
      });
    ...
    it('switches between filters correctly', async () => {
      ...
      expect(screen.getAllByText('Today Task')[0]).toBeInTheDocument();
